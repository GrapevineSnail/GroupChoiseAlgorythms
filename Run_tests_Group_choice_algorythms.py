...
button_read_n_and_m = Button(frame_n_m, **button_opts_disabled,
                             text="Ввод n и m",
...
button_forfile = Button(frame_forfile, **button_opts_disabled,
                        text="Ввод из файла",
                        command=read_file,
                        ).pack(**pack_optsLW)
...
def run_tests():
    ...
    def Linear_medians(R_list):
        ...
        median_dist = min(Distances)
        Rankings = [
            All_rankings[i] for i in range(all_rankings_cnt)
            if Distances[i] == median_dist]
        return Rankings, median_dist
